...
Route53.prototype.zonesAsync = function() {
  var self = this;
  ...
  self.zoneInfo(zoneId, function(err, zoneInfo) {
    ...
    function listResourceRecords(nextName, nextType, nextIdentifier, callback) {
      ...
      self.client.listResourceRecordSets(args, function(err, response) {
        ...
        var newRecords = convertListResourceRecordSetsResponseToRecords(
          response
        );
...
Route53.prototype.recordsAsync = function(zoneId) {
  var self = this;
  return new Promise(function(resolve, reject) {
    self.records(zoneId, function(err, records) {
      if (err) {
        reject(err);
      } else {
        resolve(records);
...
Route53.prototype.setRecord = function(opts, pollEvery, callback) {
  ...
  self.records(opts.zoneId, function(err, records) {
    ...
    records.forEach(function(record) {
      if (
        decodeAsterisk(opts.name) ===
          addTrailingDotToDomain(decodeAsterisk(record.name)) &&
    ...
    if (opts.values) {
      ...
      change.ResourceRecordSet.AliasTarget = opts.alias;
    }
    ...
    self.client.changeResourceRecordSets(args, function(err, result) {
      ...
      var changeInfo = convertChangeResourceRecordSetsResponseToChangeInfo(
        result
      );
...
Route53.prototype.setRecordAsync = function(args) {
  var self = this;
  return new Promise(function(resolve, reject) {
    self.setRecord(args, function(err, res) {
      if (err) {
        reject(err);
      } else {
        resolve(res);
